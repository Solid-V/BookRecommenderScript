...
if (isset($result_data->docs)) {
  ...
  foreach($result_data->docs as $books) {
    ...
    //$_SESSION["bookTitle"] = $books->title;
    ...
    $subject_data = json_decode($subject_json);
     ...
     if (isset($subject_data->subjects) && is_array($subject_data->subjects)) {
      ...
      foreach($curlHandles as $url => $ch) {
        ...
        $filteredBooks = array_filter($books['works'], function ($book) {
              return str_replace("/works/", "", $book['key']) != $_SESSION['bookOlid'];
        });
      ...
      echo "Book searched: " . htmlspecialchars($_SESSION["bookOlid"]) . "<br>";
